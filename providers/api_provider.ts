...
/**
 * ApiProvider registers application-level bindings into the IoC container.
 * Specifically it wires CheckoutService with the BullMQ checkoutQueue dependency
 * since CheckoutService requires manual construction (not pure @inject).
 */
export default class ApiProvider {
  constructor(protected app: ApplicationService) {}

  register() {
    this.app.container.bind(CheckoutService, async () => {
      const { checkoutQueue } = await import('#start/queue')
      return new CheckoutService(new IdempotencyService(), checkoutQueue)
    })
  }
}
