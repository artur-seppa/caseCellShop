...
![Jaeger trace list](docs/jaeger-trace-list.png)

Each dot in the scatter plot represents one request — position = time, size = duration. The list below shows all 20 most recent traces. Note that SQLite queries (`select … orders`, `select … stock_reservations`) appear as independent spans, emitted by the recovery worker running in the background.

#### Jaeger — validation error detail

![Jaeger validation error span](docs/jaeger-validation-error.png)

A `GET /api/v1/products/:id` with an invalid ULID returns 422 in **27ms**. The span carries two log events: the raw VineJS `exception` (top) and the custom `validation_error` event (bottom) added by the exception handler — which exposes `validation.fields = id` and the full `validation.details` JSON, making the Jaeger trace self-documenting without having to inspect the HTTP response.
...
### Dashboard — Prometheus UI

The Prometheus UI at **http://localhost:9090** acts as the real-time dashboard for this project. The panels below reproduce the core observability tiles you'd build in Grafana or Datadog:

#### Cache hit/miss — `casecellshop_cache_operations_total`

![Prometheus cache operations graph](docs/prometheus-cache.png)

4 series tracked per label combination (`key_type × result`): `products_item/hit`, `products_item/miss`, `products_list/hit`, `products_list/miss`. The step in the graph marks the first `GET /products` after server start (cache miss) followed by cache hits on subsequent requests.

#### Checkout outcomes — `casecellshop_checkout_total`

![Prometheus checkout total graph](docs/prometheus-checkout.png)

6 series: `accepted`, `error`, `idempotent_replay`, `product_not_found`, `stock_unavailable`, `validation_error`. Each checkout attempt increments exactly one label, making it easy to spot anomalies (e.g. a spike in `stock_unavailable` signals demand exceeding inventory).

| Panel | PromQL | What to watch |
|-------|--------|---------------|
| **Checkout rate** | `rate(casecellshop_checkout_total[1m])` | Drop → API or queue issue |
| **Checkout error rate** | `rate(casecellshop_checkout_total{status="error"}[1m])` | Spike → bug or downstream failure |
| **Stock unavailable rate** | `rate(casecellshop_checkout_total{status="stock_unavailable"}[1m])` | Spike → high demand, low stock |
| **p95 checkout latency** | `histogram_quantile(0.95, rate(casecellshop_checkout_duration_seconds_bucket[5m]))` | > 500 ms → investigate DB or Redis |
| **Cache hit rate** | `rate(casecellshop_cache_operations_total{result="hit"}[5m]) / rate(casecellshop_cache_operations_total[5m])` | < 0.7 → TTL too short or stampede |
| **Queue depth** | `casecellshop_queue_depth` | Growing → worker is behind |
| **Worker failures** | `rate(casecellshop_jobs_processed_total{result=~"failed|card_declined|erp_declined"}[5m])` | Spike → ERP/payment issues |
| **Dead man's switch** | `rate(casecellshop_checkout_total[10m]) == 0` | Zero por 10 min → servidor caiu silenciosamente |

To reproduce any panel: open **http://localhost:9090**, paste the PromQL into the **Expression** box and click **Graph**.

---

### Alerts (Prometheus alerting rules)

Alert rules are defined in [`prometheus/alerts.yml`](prometheus/alerts.yml) and loaded automatically by Prometheus via `rule_files` in `prometheus.yml`. They map directly to the conditions monitored above:
...
> **Datadog equivalent**: these same conditions translate directly to Datadog Monitor queries using the `casecellshop.*` metric namespace — the thresholds and alert logic are identical; only the DSL changes.
